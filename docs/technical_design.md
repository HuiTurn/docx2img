...
reflow. For referenced multi-paragraph notes, paragraph boxes are measured
...
logs `footnote_continuation_unresolved`.
...
changed pixels 1.486%, SSIM 0.902276 at 150 dpi.

For multiple notes on one reference page, the pagination pass flattens
definitions in reference and paragraph order, packs the shared page capacity,
then rebuilds per-note overrides for every page. This prevents definitions not
present in a chunk from being reattached in full. The code-generated
`footnote_multiple_continuation.docx` combines five paragraphs in note 1 and
thirteen in note 2. Unmodified `f923fc8` renders 1 page against Word's 2,
logs the multiple-note and overlap warnings, and measures MAE 3.616, RMSE
28.011, changed pixels 1.904%, SSIM 0.634642 on the paired page. The shared
continuation path produces Word's exact 2/2 pages at 1241x625, is
byte-deterministic, and measures mean MAE 2.532, RMSE 23.560, changed pixels
...
against Word's 2, logs `footnote_continuation_unresolved`,
`footnote_reflow_unresolved`, and `footnote_layout_overlap`, and measures MAE
4.280, RMSE 30.711, changed pixels 2.226%, SSIM 0.561279 on the paired page.
The bounded line split produces Word's exact 2/2 pages at 1241x625, is
