"""Explicit line breaks inside one footnote paragraph can continue."""
...
from tests.fixtures.gen_fixtures import make_footnote_line_continuation


def test_explicit_footnote_lines_continue_on_second_page(tmp_path, caplog):
    docx = make_footnote_line_continuation(
    ...
    assert sum(
        run.brk is not None for run in model.footnotes["1"][0].runs
    ) == 17
...
def test_explicit_footnote_line_render_is_deterministic(tmp_path):
    docx = make_footnote_line_continuation(
        tmp_path / "footnote_line_continuation.docx"
    )
    config = Config(dpi=96)
