"""Unsupported simple fields retain their cached display result visibly."""
...
from tests.fixtures.gen_fixtures import make_hyperlink_field


def test_footer_hyperlink_field_retains_cached_result_and_warns(
    tmp_path, caplog
):
    docx = make_hyperlink_field(tmp_path / "hyperlink_field.docx")
    ...
    cached = next(
        ...
        if run.text is not None and run.text.text == "Cached destination"
    )
    assert cached.props.color == (5, 99, 193)
    assert cached.props.underline
    assert (
        "header_footer_field_cached: HYPERLINK rendered cached result"
        in caplog.text
    )
...
def test_footer_hyperlink_cached_render_is_deterministic(tmp_path):
    docx = make_hyperlink_field(tmp_path / "hyperlink_field.docx")
    config = Config(dpi=96)
